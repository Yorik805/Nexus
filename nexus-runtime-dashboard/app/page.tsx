...
export default function Page() {
  ...
  useEffect(() => { if (terminalOpen && terminalRef.current) terminalRef.current.scrollTop = terminalRef.current.scrollHeight }, [terminalOutput, terminalOpen])
  ...
  const sendTerminalCommand = async () => {
    ...
    try {
      ...
      setTerminalOutput(prev => [...prev, `Error: ${exc}`])
    }
  }
  ...
  return <main className="nexus-shell">
    ...
    {terminalOpen && <div className="terminal-backdrop" role="presentation" onClick={() => setTerminalOpen(false)}><section className="terminal-modal" onClick={e => e.stopPropagation()}><div className="terminal-header"><span className="eyebrow"><Terminal size={12} /> NEXUS TERMINAL</span><button className="close-button" onClick={() => setTerminalOpen(false)} aria-label="Close terminal"><X size={17} /></button></div><div className="terminal-body" ref={terminalRef}>{terminalOutput.length === 0 && <div className="terminal-welcome">Type a command and press Enter to send to Nexus.</div>}{terminalOutput.map((line, i) => <div key={i} className="terminal-line">{line}</div>)}</div><div className="terminal-input-row"><span className="terminal-prompt">&gt;</span><input className="terminal-input" value={terminalInput} onChange={e => setTerminalInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') sendTerminalCommand() }} placeholder="Type a message..." autoFocus /><button className="terminal-send" onClick={sendTerminalCommand}>SEND</button></div></section></div>}
  </main>
}
